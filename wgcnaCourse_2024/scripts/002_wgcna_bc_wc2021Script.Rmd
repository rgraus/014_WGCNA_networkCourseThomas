...
vst <- readRDS("./data/c_vst.rds")
m_vst <- assay(vst)
# change GENENAME into entrezId
...
m_vst_entrezId <- m_vst %>% 
  data.frame() %>% 
  rownames_to_column("SYMBOL") %>%
  inner_join(fdata) %>%
  dplyr::select(ENTREZID, starts_with ("VUC")) %>%
...
```
```{r}
identical(colnames(m_vst_entrezId), 
          dt_colDataNew$Model)
```
```{r}
tail(dt_colDataNew)
...
```
```{r}
table(table(dt_colDataNewFilt$Model))
```

```{r}
dt_colDataNewFilt <- dt_colDataNew %>% filter(Model != "PBC")
```

```{r}
identical(colnames(m_vst_entrezId), 
          dt_colDataNewFilt$Sample)
```
```{r}
dt_colDataNewFilt$Model
```
```{r}
View(dt_colDataNewFilt)
```

```{r organize_data_PRAD}
# convert the ENSEMBLE based count matrix into ENTREZ based counts 
fdataSub <- fdata %>% 
  filter(ENTREZID %in% rownames(m_vst_entrezId)) 

identical(rownames(m_vst_entrezId), fdataSub$ENTREZID)
```
...
library(DESeq2)
eset <- ExpressionSet(as.matrix(m_vst_entrezId))
pData(eset) <- dt_colDataNewFilt %>% data.frame %>% column_to_rownames(var = "Sample")
fData(eset) <- fdataSub



eset %>%
  rio::export(here::here(datadir, "ExpressionSet_bc.RDS")) # change name according to your needs
...
eset_wgcnaFilt <- eset_wgcna[selected_genes,]

# extract the expression matrix
expression <- exprs(eset_wgcna)


ISA <- (cor(expression) + 1)/2^2 # intersample adjacency matrix, nice tool for outlier detection, outliers in terms of coexpression detection
...
net %>%
  rio::export(here::here(datadir, "WGCNA_network_bc.RDS"))
...
params <- list(networkType = "signed",
               ...
               deepSplit = 1,
               numericLabels = TRUE,
               pamStage = TRUE, # mehr graue bereiche in den modulen, im plotDendroAndColors plot
               pamRespectsDendro = TRUE,
               verbose = 6,
               datExpr = t(expresssion_outlier_removed))
# Choose a set of soft-thresholding powers
...
beta <- sft$powerEstimate # Berni and Renia beta <- 12; je hoeher der beta value, desto mehr werden die branches nach oben geschifted
if (is.na(beta)){
  if (params$networkType == "unsigned"){
    beta <- 6
  } else {
...
net <- rio::import(here::here(datadir, "WGCNA_network_bc.RDS"))
...
```
-**the module structure should reflect the tree split cluster**  
...
moduleTraitCor <- cor(MEs, as.matrix(traits[rownames(MEs),]), use = "p") # der Befehl funktioniert nicht!!!
...
moduleTraitPvalue <- moduleTraitPvalue[rownames(moduleTraitCor),] # rg: comment out, did not work;
...
moduleTraitCor <- moduleTraitCor %>%
  data.frame(stringsAsFactors = FALSE) %>%
  # dplyr::mutate(module = str_replace(rownames(.), "ME", "")) %>%
    # dplyr::mutate(module = rownames(.)) %>%
